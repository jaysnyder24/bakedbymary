...
export default async function Cookie({ params }) {
  ...
  return (
    <div className='mx-auto w-full max-w-[1400px]'>
      <main className='flex flex-col md:flex-row justify-start items-center md:justify-center md:items-start w-full px-10 pt-10 pb-[140px] md:p-14 md:h-[90vh] md:space-x-14 relative'>
        <ImageSelector
          image={cookie.metadata.imageUnique}
          name={cookie.name}
          slug={cookie.metadata.slug}
          breadcrumb={true}
        />
        <div className='w-full md:w-2/3 h-auto flex flex-col justify-start items-center md:items-start space-y-5 md:space-y-8'>
          <div className='w-full flex flex-col justify-start items-center md:items-start space-y-5'>
            <h1 className='font-bold font-playfair text-6xl text-pink-950'>
              {cookie.name} Cookie
            </h1>
            <span className='w-full font-poppins font-medium text-lg text-pink-600 flex flex-row justify-start items-center'>
              ${priceDecimal}
              {priceSplit.length > 2 ? '0' : '.00'} / half dozen
            </span>
          </div>
