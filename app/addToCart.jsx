    ...
    return (
        <form className={"flex flex-col justify-start items-start space-y-4 transition-all duration-600 overflow-hidden " + (props.delay ? "opacity-0 h-0 -translate-y-10 group-hover:opacity-100 group-hover:h-auto group-hover:translate-y-0" : "")} action={addToCart}>
            <div className="flex flex-row justify-between items-center space-x-2">
                <button type="push" onClick={() => setQuantity(prevQuantity => prevQuantity - 1)} disabled={quantity === 0 ? true : false} className={"p-2 rounded-full " + (props.theme === "light" ? "bg-white opacity-90 hover:opacity-100" : "bg-pink-700 hover:bg-pink-800")}><MinusIcon color={props.theme === "light" ? "#500724" : "white"} size={16} /></button>
                ...
                <button type="push" onClick={() => setQuantity(prevQuantity => prevQuantity + 1)} className={"p-2 rounded-full " + (props.theme === "light" ? "bg-white opacity-90 hover:opacity-100" : "bg-pink-700 hover:bg-pink-800")}><PlusIcon color={props.theme === "light" ? "#500724" : "white"} size={16} /></button>
            </div>
